...
async function main() {
  ...
  // When running locally on Windows from the 'contracts' directory:
  const outputPath = path.resolve(process.cwd(), '../backend/contract-addresses.json');
  fs.writeFileSync(outputPath, JSON.stringify(config, null, 2));
  console.log("Seed data applied and contracts addresses written to:", outputPath);
}
